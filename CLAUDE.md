...
- **`questions`** are seeded (`userId IS NULL`) **and** user-added (`userId` set) in the same table. A non-null `targetId` makes a question a per-company/industry variant. User-owned rows are edited/deleted in place; **seeded rows are edited via a per-user `question_overrides` overlay** (keyed by the same `questionId`, applied on read via `lib/question-overrides.ts` — so existing links/answers/cards stay valid, edits stay private, and reseeds don't clobber them). `resetQuestion` deletes the override to revert.
